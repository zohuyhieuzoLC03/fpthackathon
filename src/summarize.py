...
output_summarize = ''
message_summarize = "Hãy dùng tiếng Việt để tóm tắt một cách dễ hiểu đoạn văn sau:\n" + text

conversation_id = claude_api.create_new_chat()['uuid']
output_summarize += claude_api.send_message(message_summarize, conversation_id)
deleted = claude_api.delete_conversation(conversation_id)

sys.stdout.write(json.dumps(output_summarize))